...
string IntegerToEnglishWords::numberToWords(int num)
{
  ...
  while (num != 0) {
    ...
    if (base > 0) {
      if (ib == 0) ret = parse(base);
      else ret = parse(base) + " " + dict_b[ib] + " " + ret;
    }
    ib++;
    num /= 1000;
  }
...
string IntegerToEnglishWords::parse(int value)
{
  ...
  if (value >= 100) ret = dict_a[value / 100] + " Hundred";

  value %= 100;

  if (value == 0) return ret;
  else if (value <= 20) ret += " " + dict_a[value];
  else {
    int upper = 29;

    while (value > upper) upper += 10;
    ret += " " + dict_a[upper / 10 * 10];

    if (value - upper / 10 * 10 > 0) ret += " " + dict_a[value - upper / 10 * 10];
