...
int RectangleArea::computeArea(int A, int B, int C, int D, int E, int F, int G, int H)
{
    ...
    if (x == CONTAIN && y == CONTAIN) {
        if (sx == C - A && sy == D - B) return area(E, F, G, H);
        if (sx == G - E && sy == H - F) return area(A, B, C, D);
    }
...
{
    if (a1 > b1) return decide(b1, b2, a1, a2);
    if (a2 <= b1) return DISJOINT;
    if (b2 <= a2) return CONTAIN;
    return OVERLAP;
}
...
{
    if (a1 > b1) return share(b1, b2, a1, a2);
    return min(a2, b2) - b1;
}
