...
bool ValidateBinarySearchTree::isValidBST(TreeNode* root)
{
	...
	if (root->left == nullptr) return isValidBST(root->right) && root->val < minNodeVal(root->right);
	if (root->right == nullptr) return isValidBST(root->left) && root->val > maxNodeVal(root->left);
