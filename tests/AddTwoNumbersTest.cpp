...
TEST_CASE("Add Two Numbers") {
  ...
  SECTION("Sample test") {
    ...
    item = new ListNode(4); item->next = l1; l1 = item;
    item = new ListNode(2); item->next = l1; l1 = item;

    // l2 (5 -> 6 -> 4)
    item = new ListNode(4); item->next = l2; l2 = item;
    item = new ListNode(6); item->next = l2; l2 = item;
    item = new ListNode(5); item->next = l2; l2 = item;

    l3 = s.addTwoNumbers(l1, l2);

    // should be (7 -> 0 -> 8)
    item = l3;
    REQUIRE_FALSE(item == nullptr); REQUIRE(item->val == 7); item = item->next;
    REQUIRE_FALSE(item == nullptr); REQUIRE(item->val == 0); item = item->next;
    REQUIRE_FALSE(item == nullptr); REQUIRE(item->val == 8); item = item->next;
  ...
  SECTION("l1 is an empty list") {
    ...
    REQUIRE_FALSE(item == nullptr); REQUIRE(item->val == 6); item = item->next;
